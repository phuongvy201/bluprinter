...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Cấu hình Domain</h1>
                <p class="text-gray-600">
                    Quản lý cấu hình Currency và Google Analytics cho từng domain.
                </p>
            </div>
            <a href="{{ route('admin.settings.domain-config.create') }}" class="px-5 py-2.5 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">
                + Thêm Domain
            </a>
        </div>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($currencyConfigs as $config)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('admin.settings.domain-config.edit-currency', $config->id) }}" class="text-blue-600 hover:text-blue-900">
                                            Sửa
                                        </a>
                                        <form action="{{ route('admin.settings.domain-config.destroy-currency', $config->id) }}" method="POST" class="inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa cấu hình này?');">
                                            @csrf
                                            @method('DELETE')
    ...
    <!-- Analytics Configs Section -->
    <div class="mb-8">
        <div class="mb-4">
            <h2 class="text-xl font-bold text-gray-900">Cấu hình Google Analytics</h2>
            <p class="text-sm text-gray-600">Quản lý cấu hình Google Analytics cho từng domain</p>
        </div>
        <div class="bg-white shadow-md rounded-2xl overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($analyticsConfigs as $config)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-medium text-gray-900">{{ $config->domain }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('admin.settings.domain-config.edit-analytics', $config->id) }}" class="text-blue-600 hover:text-blue-900">
                                            Sửa
                                        </a>
                                        <form action="{{ route('admin.settings.domain-config.destroy-analytics', $config->id) }}" method="POST" class="inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa cấu hình này?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-900">
                                                Xóa
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    Chưa có cấu hình analytics nào.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
