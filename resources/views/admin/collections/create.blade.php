...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl shadow-lg p-6 text-white">
        <h1 class="text-3xl font-bold">📚 Create Global Collection</h1>
        <p class="text-purple-100 mt-2">Shared by all shops. Products auto-join by keywords and Studio AI theme matching.</p>
    </div>

    <!-- Form -->
    <form id="collection-create-form" action="{{ route('admin.collections.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-md p-8 space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label class="flex items-start space-x-3 p-4 border border-indigo-200 rounded-lg bg-indigo-50 cursor-pointer">
                        <input type="checkbox"
                               ...
                               {{ old('ai_match_products', '1') ? 'checked' : '' }}>
                        <span>
                            <span class="block text-sm font-semibold text-gray-900">Use AI to add matching products</span>
                            <span class="block text-xs text-gray-600 mt-1">Studio AI reads the collection name/description and attaches catalog products that fit the theme. Keyword matches still run first.</span>
                        </span>
                    </label>
                </div>
                ...
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="automatic" {{ old('type', 'automatic') == 'automatic' ? 'selected' : '' }}>🤖 Automatic - Match by keywords</option>
                    </select>
                    @error('type')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div class="border-b border-gray-200 pb-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                ...
                Extra Products (optional)
            </h3>
            <p class="text-sm text-gray-500 mb-4">Keyword matching runs automatically. Use this only to pin extra products that do not share keywords.</p>
            <div class="space-y-4">
    ...
    <!-- Product Selection Modal (outside form so Enter/search cannot submit create) -->
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] flex flex-col">
            <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-t-xl p-6 text-white">
                <div class="flex justify-between items-center">
                    ...
                    <button type="button" onclick="closeProductModal()" class="text-white hover:text-gray-200 transition">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody id="products-table-body" class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="product-row hover:bg-purple-50 transition"
                                    ...
                                    data-category-id="{{ optional($product->template)->category_id ?? '' }}"
...
<script>
    ...
    function filterProducts() {
        ...
        rows.forEach(row => {
            ...
            if (priceFilter) {
                ...
                if (price < min || (max !== Infinity && price > max)) visible = false;
            }

            if (categoryFilter && categoryId !== categoryFilter) visible = false;
            if (templateFilter && templateId !== templateFilter) visible = false;
            if (shopFilter && shopId !== shopFilter) visible = false;
    ...
    function toggleSelectAll() {
        ...
        Array.from(document.querySelectorAll('.product-row'))
            ...
            .forEach(row => {
                ...
                if (selectAll) selectedProducts.add(parseInt(checkbox.value));
                else selectedProducts.delete(parseInt(checkbox.value));
            });
    ...
    function updateSelectedProducts() {
        document.querySelectorAll('.product-checkbox').forEach(checkbox => {
            if (checkbox.checked) selectedProducts.add(parseInt(checkbox.value));
            else selectedProducts.delete(parseInt(checkbox.value));
        });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Restore old selected products into hidden inputs after validation redirect
        const container = document.getElementById('selected-products-inputs');
        container.innerHTML = '';
        selectedProducts.forEach(productId => {
            const input = document.createElement('input');
