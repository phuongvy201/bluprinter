            ...
            <div class="flex flex-col w-64 bg-white border-r border-gray-200 shadow-sm fixed left-0 top-0 h-screen overflow-y-auto sidebar-scroll z-40">
                ...
                <nav class="flex-1 px-4 py-6 space-y-1">
                    ...
                    @if(auth()->user()->hasRole('admin'))
                        ...
                        <a href="{{ route('admin.settings.domain-currency.index') }}" 
                           class="group flex items-center px-3 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.settings.domain-currency.*') ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                            <svg class="w-5 h-5 mr-3 flex-shrink-0 {{ request()->routeIs('admin.settings.domain-currency.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
