...
return new class extends Migration
{
    ...
    {
        Schema::create('shop_gmc_configs', function (Blueprint $table) {
            $table->id();
            $table->foreignId('shop_id')->constrained()->onDelete('cascade');

            // Config name for easy identification
            $table->string('name')->comment('Tên config để dễ nhận biết (VD: UK Store, US Store)');
            ...
            // Unique constraint: mỗi shop chỉ có 1 config cho mỗi target_country
            $table->unique(['shop_id', 'target_country'], 'shop_country_unique');
    ...
    {
        Schema::dropIfExists('shop_gmc_configs');
    }
};
